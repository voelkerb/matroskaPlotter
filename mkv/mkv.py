...
def seekLoad(filepath, seekPos, length, verbose=False, streamsToLoad=None, titles=None):
    ...
    import pysubs2
    if seekPos < 0:
        printWarning("seekpos smaller 0" + str(seekPos))
        seekPos = 0
    # PTS is in milliseconds
    start_pts = int(seekPos*1000)
    end_pts = start_pts + int(length*1000)

    if verbose: print("Seek from: " + str(start_pts) + "ms to " + str(end_pts) + "ms")
        ...
        for frame in container.decode(streams=stream.index):
            ...
            if not initCheck:
                if frame.pts > start_pts:
                    printError("seeked too far " + str(frame.pts) + " instead of " + str(start_pts))
                    sys.exit()
                initCheck = True
...
def makeMetaArgsFromDict(dic, type, stream=0):
    ...
    if type[0] not in ["s", "a", "v"]:
        printError("Currently only \"audio\", \"subtitles\" or \"video\" is allowed as type")
        return ""
